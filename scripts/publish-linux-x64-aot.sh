#!/bin/bash
# Publish Native AOT executable for Linux x64
...
OUTPUT_PATH="$SCRIPT_DIR/../publish/linux-x64-aot"

echo -e "\033[36mPublishing Outline.Mcp.Server for Linux x64 (Native AOT)...\033[0m"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "\033[37m    ./publish/linux-x64-aot/Outline.Mcp.Server\033[0m"
else
    echo -e "\n\033[31m[FAIL] Build failed!\033[0m"
    echo -e "\n\033[33mNote: Native AOT requires compatible code. If build fails:\033[0m"
    echo -e "\033[33m  - Use standard self-contained build instead (publish-linux-x64.sh)\033[0m"
